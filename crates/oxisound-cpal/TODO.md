...
- [ ] Evaluate `crossbeam-utils` cache-line padding overhead in ringbuf: measure false sharing impact on ARM vs x86 (~analysis) — **Analysis-only:** ringbuf 0.5.0 uses `cache-padded` internally for producer/consumer pointers. False sharing between the audio callback thread (consumer) and the write thread (producer) is mitigated. Benchmarking on specific hardware is needed to quantify; no code change expected.
- [ ] Profile async input stream mpsc channel overhead vs ring buffer approach (~analysis) — **Analysis-only:** async input uses `tokio::sync::mpsc` with bounded capacity (128 chunks). The ring buffer path has lower per-sample overhead but the mpsc approach integrates with tokio's executor. Acceptable for capture (non-realtime consumers); not acceptable for low-latency processing. Defer.
